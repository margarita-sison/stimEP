function struct = ms_getepochs(struct, time_window, signal_type, sampling_rate)
...
sampling_rate = struct.(sampling_rate);
% -----
...
% -----
epochs = zeros(size(signal_matrix,1), length(peaks), length(pre_onset_len:post_onset_len)); % channels x epochs x samples
...
for c = 1:size(signal_matrix,1)
    ...
    for p = 1:length(peaks)
        ...
        if peak+pre_onset_len < 1 || peak+post_onset_len > length(chan) % skip if there are insufficient points before/after event onset
            continue
        end

        epoch = chan(round(peak)+pre_onset_len:round(peak)+post_onset_len); % pre_onset_len has negative val
        epochs(c,p,:) = epoch; % epoch for channel 'c' around event onset 'o'
    end
end

struct.time_window = time_window;
struct.(append(signal_type,'_epochs')) = epochs;
end